...
/*            joint_matrix_load(sg, b_frag, multi_ptr< half, sycl::access::address_space::global_space>(b) + bRow  + bCol * ldb , ldb );*/
            ...
            joint_matrix_load(sg, b_frag, multi_ptr< half, sycl::access::address_space::global_space>(b) + bRow * ldc + bCol * 2, ldc * 2 );            
...
//    auto wi_data_acc = get_wi_data(sg, acc_frag);
    ...
    if (cRow < m_ld && cCol < n_ld) {
        //    wmma::load_matrix_sync(c_frag, c + cCol + cRow * ldc, ldc,
          //                         wmma::mem_row_major);
	  auto wi_data_acc = sycl::ext::intel::experimental::matrix::get_wi_data(sg, acc_frag);
        joint_matrix_load(sg, c_frag, multi_ptr< float, sycl::access::address_space::global_space>(c) + cCol + cRow * ldc, ldc, layout::row_major);
          auto wi_data_c = sycl::ext::intel::experimental::matrix::get_wi_data(sg, c_frag);
//	  auto wi_data_acc = get_wi_data(sg, acc_frag);
...

{
    ...
    */
    ...
    vnniB_h = (sycl::half*)malloc(sizeof(sycl::half) * K_GLOBAL * N_GLOBAL);
...
//    sycl::half* B = NULL;
  ...
  /*  B = (sycl::half*)sycl::malloc_device(
    ...
    vnniB = (sycl::half*)sycl::malloc_device(
        sizeof(sycl::half) * N_GLOBAL * K_GLOBAL, dpct::get_default_queue());
    /*
    ...
    */
    ...
    assert(((unsigned long long)vnniB) % 128 == 0);
    ...
    */
    ...
    may need to rewrite this code.
    */
    matrix_transpose<half>(K_GLOBAL, N_GLOBAL, B_h, B_t); //B_t[i,j] = B_h[j,i]
    matrix_vnni<half>(K_GLOBAL, N_GLOBAL, B_t, vnniB_h, 2);

/*    dpct::get_default_queue()
        .memcpy(B, B_h, sizeof(sycl::half) * N_GLOBAL * K_GLOBAL)
        .wait();*/
    dpct::get_default_queue()
        .memcpy(vnniB, vnniB_h, sizeof(sycl::half) * N_GLOBAL  * K_GLOBAL)
        .wait();
...
/*for(int k=0;k<K_GLOBAL * N_GLOBAL; k++)
{
	std::cout << " Host side vnniB_h matrix values after vnni packed: vnniB_h[" << k << "] = " << vnniB_h[k] << std::endl;
	std::cout << " Host side B matrix values: B_h[" << k << "] = " << B_h[k] << std::endl;
}*/
    ...
    */
    //checkCudaErrors(
    (sycl::free(vnniB, dpct::get_default_queue()));
    /*
